...
import (
	...
	pkgdrivers "github.com/machine-drivers/docker-machine-driver-nobidev/pkg/drivers"
	"regexp"
...
const (
	...
	pidFileName     = "nobidev-docker.pid"
	machineFileName = "nobidev-docker.json"
...
func (d *Driver) Start() error {
	h, err := nobidev.New("", "", filepath.Join(d.StorePath, "machines", d.MachineName))
	if err != nil {
		return err
	}
...
func (d *Driver) nfsExportIdentifier(path string) string {
	return fmt.Sprintf("minikube-nobidev %s-%s", d.MachineName, path)
}
